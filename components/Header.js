...
export default function Header() {
    ...
    const clickDocumentHandler = event => {
        if (window.scrollY < 100 && window.innerWidth >= globalState.sizes.lg) return
        setIsRabbitFixed(true)
        setIsHeaderFixed('fixed')
        refIsHandled.current = false
        document.removeEventListener('scroll', clickDocumentHandler)
    }
    ...
    const hoverEnterHandler = event => {
        ...
        setTimeout(() => {
            document.addEventListener('scroll', clickDocumentHandler)
        }, 300)
    }
