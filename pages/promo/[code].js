...
import { promo as categories } from '../../components/helpers/categories'

export default function PromoPage({detail}) {
    const c = detail.currentCategory

    return (
        <MainLayout title={`Акция | ${c.name}`}>
            <Breadcrumbs theme={c.theme ? c.theme : false} link={['Со скидкой|/promo', `${c.name}|/test`]} />
